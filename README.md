...
For games that need a keyboard instead -- Typing of the Dead on the Dreamcast, for one -- the same Pico can switch to a USB keyboard with `couchlink.exe keyboard` and forward the player's typing. A Pico wired for Dreamcast Maple output can use `couchlink.exe maple` to translate the same Parsec/XInput controller stream into a Maple controller. See [Controller Routing](https://github.com/RealWhyKnot/ParsecCouchLink/wiki/Controller-Routing) for keyboard and Maple modes.
