...
$TempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("ParsecCouchLink.ReleaseNotes." + [Guid]::NewGuid().ToString('N'))
...
- **Changelog:** <https://github.com/{full-repo}/blob/main/CHANGELOG.md>
...
Download `{zip-name}` and run setup.
'@
    ...
    Invoke-Git tag -a v2026.5.1.0 -m 'ParsecCouchLink 2026.5.1.0'

    Commit-TestChange `
        -Path 'src.txt' `
        -Content "plain-prerelease`n" `
        -Subject 'fix(helper): plain prerelease fallback'
    Invoke-Git tag -a v2026.5.2.0 -m 'ParsecCouchLink 2026.5.2.0'

    Commit-TestChange `
        -Path 'src.txt' `
        -Content "beta`n" `
        -Subject 'fix(relay): beta port fallback'
    Invoke-Git tag -a v2026.5.2.1-beta -m 'ParsecCouchLink 2026.5.2.1-beta'

    Commit-TestChange `
        -Path 'src.txt' `
        -Content "stable`n" `
        -Subject 'fix(vrclog): stable startup tail'
    Invoke-Git tag -a v2026.5.3.0 -m 'ParsecCouchLink 2026.5.3.0'

    $TemplateDir = Join-Path $TempRoot '.github/release-template'
    $StableNotes = (& $Generator `
            -Tag v2026.5.3.0 `
            -Repo RealWhyKnot/ParsecCouchLink `
    ...
    $zipPath = Join-Path $TempRoot 'artifacts/ParsecCouchLink-v2026.5.3.0.zip'
    $manifestPath = Join-Path $TempRoot 'artifacts/ParsecCouchLink-v2026.5.3.0.manifest.tsv'
    $zipSha = 'E8966F33BE8246922756E3E8234CF8309FB6D3151665594203F53BBF5725164B'
    Write-TestFile -Path 'artifacts/ParsecCouchLink-v2026.5.3.0.zip' -Content 'zip'
    Write-TestFile -Path 'artifacts/ParsecCouchLink-v2026.5.3.0.manifest.tsv' -Content "path`tbytes`tsha256`ncouchlink.exe`t123`tA24EA7D3DF2B0718AFF60B5B9EBEBDF590ED4938D81A6B08CDEC7A880B326B0C`n"
    ...
    $IntegrityNotes = (& $Generator `
            ...
            -PrereleaseTags v2026.5.2.0 `
            -ZipPath $zipPath `
            -ZipName 'ParsecCouchLink-v2026.5.3.0.zip' `
            -ZipSize 157017922 `
            -ZipSha256 $zipSha `
