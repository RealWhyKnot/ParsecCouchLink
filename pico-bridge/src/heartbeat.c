...
static bool due(void) {
    if (absolute_time_diff_us(get_absolute_time(), next_beat) > 0) return false;
    next_beat = make_timeout_time_ms(HEARTBEAT_INTERVAL_MS);
    beat_count++;
    return true;
}
...
void heartbeat_run_mode_task(void) {
    ...
    if (!assoc_result_logged) {
        ...
        bool was_joining = (assoc_last_observed == WIFI_STATE_JOINING
                            || assoc_last_observed == WIFI_STATE_IDLE);
        bool settled = (ws == WIFI_STATE_JOINED || ws == WIFI_STATE_FAILED);
        if (was_joining && settled) {
            diag_log_printf("hb#%u assoc_result=%s last_error=%u",
                            (unsigned)beat_count,
                            wifi_state_name(ws),
                            (unsigned)wifi_last_error_code());
    ...
    diag_log_printf(
        "hb#%u t=%us mode=run usb=%s%s wifi=%s ip=%u.%u.%u.%u rssi=%d peer=%s "
        "since_peer_ms=%u pcon=%d tx_pkts=%u rx_pkts=%u",
        (unsigned)beat_count, (unsigned)uptime_s,
        usb_up ? "mounted" : "unmounted",
        usb_susp ? "/suspended" : "",
        wifi_state_name(ws),
        (unsigned)((ip >> 0) & 0xFF),
        (unsigned)((ip >> 8) & 0xFF),
        ...
        (unsigned)((ip >> 24) & 0xFF),
        (int)rssi_val,
        have_peer ? "latched" : "none",
        (unsigned)since_peer,
        (int)g_parsec_connected,
        (unsigned)net_udp_tx_count(),
...
void heartbeat_setup_mode_task(void) {
    ...
    diag_log_printf(
        "hb#%u t=%us mode=setup usb=%s%s cdc_dtr=%d",
        (unsigned)beat_count, (unsigned)uptime_s,
        usb_up ? "mounted" : "unmounted",
        usb_susp ? "/suspended" : "",
        (int)dtr);
